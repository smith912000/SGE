...
function ResonanceIndicator({ res }) {
  if (!res || !res.is_369) return null;
  return (
    <span title={`Vortex Root: ${res.digital_root}`}
      style={{ fontSize: "0.55rem", background: M3.tertiary + "22", color: M3.tertiary, padding: "1px 4px", borderRadius: 4, border: `1px solid ${M3.tertiary}44`, marginLeft: 6 }}>
      {res.digital_root}
    </span>
  );
}
...
export default function AspectTable({ aspects }) {
  const TH = {
    padding: "5px 10px", textAlign: "left", color: M3.secondary,
            ...
            return (
              <tr key={i} style={{ borderBottom: `1px solid ${M3.outlineVariant}22` }}>
                ...
                <td style={TD}>
                  ...
                  <ResonanceIndicator res={a.resonance} />
                </td>
